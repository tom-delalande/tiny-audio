...
static const clap_plugin_descriptor_t s_c99dist_desc = {
    ...
    .id = "io.tinyclub.clap-plugin",
    .name = "Custom Clap Plugin",
...
enum ClipType { HARD = 0, SOFT = 1, FOLD = 2 };

enum ParamIds { pid_DRIVE = 2112, pid_MIX = 8675309, pid_MODE = 5150 };
...
static void c99dist_process_event(clap_c99_distortion_plug *plug,
...
static uint32_t c99dist_audio_ports_count(const clap_plugin_t *plugin,
                                          bool is_input) {
  return 1;
}

static bool c99dist_audio_ports_get(const clap_plugin_t *plugin, uint32_t index,
                                    bool is_input,
...
bool c99dist_param_get_value(const clap_plugin_t *plugin, clap_id param_id,
                             double *value) {
...
bool c99dist_param_value_to_text(const clap_plugin_t *plugin, clap_id param_id,
...
bool c99dist_text_to_value(const clap_plugin_t *plugin, clap_id param_id,
...
bool c99dist_state_save(const clap_plugin_t *plugin,
                        const clap_ostream_t *stream) {
  clap_c99_distortion_plug *plug = plugin->plugin_data;

  // We need to save 2 doubles and an int to save our state plus a version. This
  // is, of course, a terrible implementation of state. You should do better.
...
bool c99dist_state_load(const clap_plugin_t *plugin,
...
static bool c99dist_activate(const struct clap_plugin *plugin,
...
static bool c99dist_start_processing(const struct clap_plugin *plugin) {
  return true;
}
...
static const void *c99dist_get_extension(const struct clap_plugin *plugin,
                                         const char *id) {
